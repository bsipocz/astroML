...
def compute_XD_results(n_points=2000, n_components=10, max_iter=500, threading=False):
    ...
    x_true = (1.4 + 2 * np.random.random(n_points)) ** 2
    y_true = 0.1 * x_true ** 2
    ...
    x_true += np.random.normal(0, dx, n_points)
    y_true += np.random.normal(0, dy, n_points)

    # add noise to get the "observed" distribution
    dx = 0.2 + 0.5 * np.random.random(n_points)
    dy = 0.2 + 0.5 * np.random.random(n_points)
